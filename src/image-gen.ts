 ...
 *   sk-or → OpenRouter (FLUX.2 Klein via chat completions)
 *   r8_   → Replicate (FLUX Schnell)
 *   key-  → Fal.ai (FLUX Schnell)
 *   else  → Together AI (FLUX Schnell)
 *
 * Returns the absolute path to a temporary image file.
 */
export async function generateImage(apiKey: string, prompt: string): Promise<string> {
  const outPath = join(tmpdir(), `openclaw-img-${Date.now()}.webp`);
  ...
  if (apiKey.startsWith("fw_")) {
    ...
    try {
      ...
      while (true) {
        ...
        // else Pending or "Task not found" (queuing) — keep polling
      }
    } finally {
      clearTimeout(timer);
    }
    ...
    try {
      ...
      const data = (await res.json()) as {
        choices?: { message?: { content?: string; images?: Array<string | { image_url?: string | { url?: string } }> } }[];
      };
