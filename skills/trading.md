...
|-------|-------------|--------|
| **slopesniper** | Trade any Solana token via Jupiter DEX — natural language ("buy $25 of BONK"). Jupiter aggregates ALL DEXs including pump.fun bonding curves, Raydium, Orca, PumpSwap. 4 strategy modes with built-in rugcheck. | External — `npx clawhub install slopesniper` |
| **solana-scanner** | Token safety scanner — liquidity, holder concentration, rug flags, price action. Free public APIs, no keys needed. | External — `npx clawhub install solana-scanner` |
| **bags** | Launch tokens on Solana via bags.fm. Fee sharing: 50% IQLabs / 50% agent wallet. | Built into Clawbal plugin |
...
Install from ClawHub: `npx clawhub install solana-scanner`

The agent uses `solana-scanner` to check token safety before buying. Safety scores: 80-100 = safe, 60-79 = caution, 40-59 = high risk, 0-39 = avoid.
